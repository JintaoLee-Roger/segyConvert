...
void SegyFile::getBinaryHeader(){
    ...
    binaryHeader_.numTrailer = swap_endian<int32_t>(readData<int32_t>(3200+328));
    in_.seekg(0, std::ios::end);
    uintmax_t flength = in_.tellg();
    binaryHeader_.totalTrace = (flength - (3600 + binaryHeader_.numExHeader * 3200)) / (240 + binaryHeader_.ns * 4);
}
...
void SegyFile::toDat(const std::string outfile){
    ...
    std::cout << "Total traces: " << totalT << "    Segy file only contains "
              ...
              << (totalT - binaryHeader_.totalTrace) << " zero traces to pad it" << std::endl;
    ...
    if (idx1 != binaryHeader_.totalTrace){
        std::cout << "Error! There are " << binaryHeader_.totalTrace
                  << " traces in Segy file, but read "
                  << idx1 << " traces." << std::endl;
        exit(1);
    }
        ...
        std::cout << idx1 << " traces have been read from segy file and written to the binary file."
                  << std::endl
                  << idx2 << " traces with zero have been padded." << std::endl
                  << binaryHeader_.totalTrace << " traces in segy file." << std::endl
