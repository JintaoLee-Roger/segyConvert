...
void SegyFile::guessLoc() {
    ...
    for (size_t i = 0; i < 3; ++i) {
        int inl1 =
            swap_endian<int32_t>(readData<int32_t>(start1 + inline_loc[i]));
        int inl2 =
            swap_endian<int32_t>(readData<int32_t>(start3 + inline_loc[i]));
    ...
    for (size_t i = 0; i < 3; ++i) {
        int xl1 =
            swap_endian<int32_t>(readData<int32_t>(start1 + xline_loc[i]));
        int xl2 =
            swap_endian<int32_t>(readData<int32_t>(start3 + xline_loc[i]));
        // printf("xl1: %d, xl2: %d\n", xl1, xl2);
...
void SegyFile::toDat(const std::string outfile) {
    ...
    for (size_t i = th.inmin; i <= th.inmax; ++i) {
        for (size_t j = th.xlmin; j <= th.xlmax; ++j) {
            if (iline == i && xline == j) {
                ...
                iline =
                    swap_endian<int32_t>(readData<int32_t>(start + th.inlLoc));
                xline =
                    swap_endian<int32_t>(readData<int32_t>(start + th.xlLoc));
            } else {
    ...
    if (idx1 != binaryHeader_.totalTrace) {
        ...
        std::cout << idx1
                  ...
                  << std::endl
                  << "Totally, " << totalT
                  << " traces have been written to the binary file."
                  << std::endl;
    }
...
void SegyFile::scan() {
    ...
    start =
        start + (240 + binaryHeader_.ns * 4) * (binaryHeader_.totalTrace - 1);
