...
public:
    ...
    void guessLoc();
    // scan the whole segy file, obtain the range of in-line and cross-line number
    void scan();
...
private:
    ...
    // read a specific format number from a specific loaction, used in binary or trace header
    template <typename T>
    T readData(int64_t loc);
};

template <typename T> T SegyFile::swap_endian (T u){
...
template <typename T> T SegyFile::readData(int64_t loc){
